...
const SidebarNavItem = ({ item, isCollapsed, isActive, onClick }) => {
  ...
  const activeClasses = "bg-white border-r-4 border-primary text-primary";
  const inactiveClasses = "text-secondary hover:bg-gray-50";
...
const DashboardHeader = ({ onMenuClick, isOpen }) => (
  <header className="fixed md:hidden top-0 left-0 right-0 h-14 flex items-center justify-between px-4 bg-secondary shadow z-50">
    <Logo w="32" />
    <button aria-label="Toggle menu" onClick={onMenuClick} className="p-1">
      {isOpen ? <X color="white" className="w-6 h-6" /> : <Menu color="white" className="w-6 h-6" />}
    </button>
  </header>
);
...
const DashboardSidebar = ({
  ...
  return (
    ...
    >
      ...
      <div
        className={`hidden bg-secondary md:flex items-center border-b p-4 ${
      ...
      {/* Nav Items */}
      <nav className="flex-1 bg-secondary-light flex flex-col justify-between">
